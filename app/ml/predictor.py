...
def get_days_of_data():
    first = Transaction.query.order_by(Transaction.timestamp.asc()).first()
    last = Transaction.query.order_by(Transaction.timestamp.desc()).first()
    if not first or not last:
        return 0
    return (last.timestamp.date() - first.timestamp.date()).days

def prepare_product_features(product_df):
    """Prepare ML features for a single product"""
...
def train_model(daily_df):
    """Train Random Forest on product daily sales"""
    feature_cols = [
    ...
    # Train test split (85/15 time based)
